...
Launch a pentest against the serverless / FaaS tier fronting {{TARGET}}:
Lambda, Azure Functions, Google Cloud Functions, Cloudflare Workers or OpenFaaS.
Reason at the scale of the WHOLE serverless tier, not of a single function. The
objective is twofold: ABUSE THE FUNCTION ITSELF (handler injection, event-data
poisoning, framework deserialization, SSRF to the cloud metadata service, IAM
role escalation through an over-privileged execution role) and PIVOT OFF THE
COLD START (exfiltrate the execution role credentials from the injected function,
then abuse the role's permissions beyond the function boundary to reach neighbour
cloud resources — S3 buckets, DynamoDB tables, Secrets Manager entries).
PROVE each path end to end with the exact invocation payload and its raw response,
headers included. Use curl, aws, az, gcloud, jq, ffuf and nuclei.

STRICT CONSTRAINTS:

- Operate only against the provided hostnames, ARNs, function names and cloud
  accounts. A neighbour resource you discover is in scope ONLY if it belongs to the
  engagement; never probe a third-party tenant or a shared-tenancy neighbour.
- Read and enumerate first. A state-changing cloud call (IAM policy deploy, function
  code rewrite, secret rotation, bucket policy change) is allowed only as the
  minimal reversible proof of a finding, and must be reverted in the same phase.
- No dependency installation. curl, aws, az, gcloud, jq, ffuf and nuclei already
  exist in the toolbox. naabu is the only port scanner available.
- No denial-of-service. Concurrency probes (cold-start flooding, latency
  measurement) are single-shot or bounded at 10 invocations, never a flood loop.
- No credential spraying against a cloud console, a control plane or a function
  authorizer. Prove an auth weakness with <=11 requests, then stop that vector.
- No theoretical explanations. Exploitation proof required: the exact invocation
  payload and its raw response.
